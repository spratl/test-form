...
// ---------- Auto block-letters free-text input (all caps, matches paper-form convention) ----------
document.getElementById('dataForm').addEventListener('input', (e) => {
  ...
  const val = el.value;
  if (!val) return;
  const blockCased = toBlockCase(val);
  if (blockCased !== val) {
...
// Converts free text to block letters (all caps) — used both live as the candidate types and
// again here as a safety net when building the PDF data, so formatting is correct even if a
// field's value got in some other way (paste, autofill, or a mobile browser that doesn't fire
// the same input events while typing).
function toBlockCase(str){
...
function collectFormData(){
  ...
  form.querySelectorAll('input[name], select[name], textarea[name]').forEach(el => {
    ...
    const isFreeText = (el.tagName === 'TEXTAREA') || (el.tagName === 'INPUT' && el.type === 'text');
    data[el.name] = isFreeText ? toBlockCase(el.value) : el.value;
  });
  // repeating tables
  ...
  tableIds.forEach(tableId => {
    ...
    document.querySelectorAll('#' + tableId + ' tbody tr').forEach(tr => {
      ...
      tr.querySelectorAll('input, select').forEach(inp => {
        row[inp.dataset.col] = inp.tagName === 'INPUT' ? toBlockCase(inp.value) : inp.value;
